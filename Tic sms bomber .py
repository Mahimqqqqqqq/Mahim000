...
rp= 'CYBER'
x = 'ARAFAT'
if 'Ihc'==user and 'Tic'==pas:
	Axak(g+"\n\n\t\t      LOGIN SUCCESSFUL JOIN MY FACEBOOK GROUP ")
	os.system('xdg-open https://t.me/+PDGtq7eBHUI1Yjll')
else:
...
number=input(f"{c}\n[ VICTIM NUMBER ] :{w} +880")
amo=int(input(c+"\n[ AMOUNT ] : "+w))
xak(f"\n\n\t {w}<{r}/{w}> {g}STAY WITH Team Tech Insaf Channel  ;) {w}<{r}/{w}>\n\n")
...
for i in range(amo):
	...
	print(f"\n\t\t {ran2}SMS SENT SUCCESFULLY💚  BY 🔥Tic 🔥 "+w+tim)
